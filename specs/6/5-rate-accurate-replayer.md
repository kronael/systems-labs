...
timeout. The reported latency of an event is the time a client arriving at that
event's due time would have waited, and the distribution over those values is
what the report summarizes. Latencies in one run span microseconds to tens of
seconds; the report states percentiles to p99.99 and the maximum, with a
declared value precision that holds across that whole span.
...
- **wrk** saturates a fixed set of threads and connections and reports the
  ...
  connections, and duration but no rate, so the load it offers is a consequence
  of the target's speed rather than an input to the run.
- **tcpreplay** holds a schedule faithfully — original timing, a fixed rate, or
  line rate — by never being a client at all: it pushes captured packets one
  way, expects no responses, and measures nothing, which is the opposite trade.
- **k6** turns the coupling between one iteration's completion and the next
  one's start into a per-scenario configuration choice, so whether a test's
