...
A practitioner would not have run this fleet bare-handed against a live
coordinator, and that is exactly the trouble with naming what they would have
reached for: at this lab's boundary every genuine neighbour — the certified
proxy stacks whose one job is keeping fleets inside their authorizations, the
protections other regulated venues document on their own side — carries the
lab's answer in its name or on its landing page. Naming a neighbour must
orient without solving, and here no candidate does the first without the
