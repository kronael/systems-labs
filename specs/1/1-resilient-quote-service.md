...
Design and build a network service that asks two independent providers for a
quote and returns the best usable result. The service must remain predictable
when demand exceeds capacity or one provider becomes slow.
...
The system accepts a quote request and returns the best valid quote with its
source and freshness. When no valid result exists, the caller receives a typed
non-2xx response. Health distinguishes a live process from a service ready to
accept work.
...
capacity. Latency and rejection are judged against the calibrated capacity and
the learner's declared service level, not a fixed number.
...
accepted work. Under sustained overload, memory and waiting work remain within
the submitted structural bounds.
...
injection are prepared. Databases, caches, retries, Kubernetes, and browser UI
