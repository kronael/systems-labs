...
summary: |
  ...
  - S1, S27, S28, S29 are SETTLED: all four NOT REPRODUCIBLE, proved from
    vendor pages and AWS's own emulator source. S32 is the one decision that
    answers them — the emulator owes the API, the controller owes the limit
  ...
  - the CEO lens was discarded: it asked for a buyer and a revenue model, and
    this is a learning repository with neither. Nothing from it shipped.
---
...
Novice-reader audit landed: three high findings. The front door stated no entry
skills, no machine requirement, and no useful first action while nothing runs.
README now carries all three, and `#what-you-will-need` is the check.

## 18:45

Phases 1 and 2 were the only spec entry points that said they are
author-facing. The other five phase READMEs and `0/5` say it now, and
`specs/index.md` states that no lab is built.

## 18:50

Seven findings stay open by design: every one is a redesign, and this repo's
own rule is that the user signs off before a redesign ships. Nothing pushed —
pushing needs the owner to ask.

## 19:05

Added root `HOWTO.md`: the learner's method, counterpart to `CONTRIBUTING.md`.
The seven-step loop, the three gates, what goes in `ARCHITECTURE.md` and when,
the Make vocabulary, the stuck ladder, and what finished looks like.

## 19:10

README said the redesign is "the part you are paying for". Nobody pays for
anything here. `8/3`'s coverage shortfall is structural now — one pass at the
ceiling costs 10,000 s and the window is shorter — so S30 is partial.

## 19:40

Three codex deep dives, read-only. AWS's RIE implements `Freeze` as a metrics
stub — returns `CycleDeltaMetrics`, suspends nothing — so the freeze three
phase-2 labs teach never happens locally. Read the source at commit `c622c06`.

## 19:45

Same verdict for the other three. The cgroup freezer "may take some time", so
observe-then-suspend cannot hold a boundary. `nanosleep(2)` warns of delay
after the sleep completes, so service times cannot supply the latency oracle.
DynamoDB Local throws no `TransactionConflictException`, which pulls `2/3` in
beside `4/1`.

## 19:50

S32 records the reframe with prior art: Toxiproxy puts the limit in a proxy,
Maelstrom checks histories rather than outputs, the TigerBeetle VOPR stubs the
clock. Transport, process, clock — three layers, one decision, four findings.

## 19:55

S26 fixed and pruned. The phase-4 corpus aim was wrong, not unmet: share the
generator and the record contract, keep each lab's numbers. `2/4` regained the
three invariants it had dropped from `1/5`; `2/3` gained the boundary-meeting
interval. S30's proposal replaces a duration with a constraint, because
200,000 x 0.06 / 3,600 = 3 1/3 pages/s against 20 does not force contention.

## 20:10

Added root `TODO.md`: what does not exist, against `BUGS.md`'s what is wrong.
A grep over all 34 specs found no lab that compares two clock readings under a
moving clock, runs the learner's code at two versions at once, or has a
dependency that discards accepted work.

## 20:15

Three proposals grounded and quoted: Cloudflare's leap second, the Kubernetes
version skew policy, vLLM's RECOMPUTE preemption. Multi-tenant fairness and
deletion propagation are marked do-not-author — no source yet, and the repo's
own rule is behaviour first, puzzle second.

## 20:40

Shipped all seven findings; `BUGS.md` is empty. The contract came first in
`e784d82`: a barrier holds execution rather than being noticed after the fact,
and the controller supplies the limit at a transport, process, or clock layer.
`1/5` declares an adapter per crash barrier in the same commit.

## 20:45

S1 and S28's phase-2 half in `d0415ff`. The freeze comes from the process layer
at the platform's own barrier, because AWS's emulator implements freeze as a
metrics stub. `2/3` and `2/4` asked for a conflict rate the local store never
produces; the transport layer records contention instead, so it reads the same
for every correct design. `2/2` needed nothing — it names no freeze.

## 20:50

S28's store half and S29 in `8028020`. `4/1` gets a declared capacity model in
front of the store and says its gate does not prove hosted partition
adaptation. `6/5` computes latency from observation on one proven-common
monotonic base, with the deadline winning ties.

## 20:55

S31 in `eca3094`: the track admits a fifth shape, the chain transaction client,
and `7/4` sits in it; `7/2` offers 0.5 x r / m against the node's own rate.
S30 in `567cd55`: the freshness declaration is admissible only when
K + R + E <= A(F) < K + R + E + U. S26 closed earlier in `38efa16`.
